...
func (f fakeSys) LookPath(cmd string) (string, error) { if f.look[cmd] { return "/bin/"+cmd, nil }; return "", errors.New("not found") }
func (f fakeSys) CurrentUser() (*user.User, error) { if f.usr != nil { return f.usr, nil }; return &user.User{Username:"me", Uid:"1000"}, nil }
func (f fakeSys) GOOS() string { return f.goos }
func (f fakeSys) GOARCH() string { return f.arch }
func (f fakeSys) Stat(name string) (os.FileInfo, error) { if f.files[name] { return fakeFileInfo{name: name}, nil }; return nil, errors.New("no") }
